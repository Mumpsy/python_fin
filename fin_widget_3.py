...
class FinplotWidget(QWidget):
    def __init__(self, parent, symbol, df):
        ...
        # y axis
        self.ax_left = self.fplt_widget.getAxis("left")
        #self.ax_left.setPen(style=Qt.DotLine)

        # Show the grid
        self.fplt_widget.set_visible(xgrid=True, ygrid=True)
        self.layout.addLayout(self.labels)
        self.layout.addWidget(self.fplt_widget.ax_widget)
        self.window().axs = [self.fplt_widget]  # required property of window
        self.setLayout(self.layout)
        self.load_df(symbol, df)

    def load_df(self, symbol, df):
        self.fplt_widget.reset()
        if len(df.index) != 0:  # Ensure there is data in the dataframe
            df_cols_list = ["Open", "Close", "High", "Low"]
            candlesticks = fplt.candlestick_ochl(df[df_cols_list], ax=self.fplt_widget)
...
class MainWindow(QMainWindow):
    def __init__(self, parent, df_dict, df_dict2):
        ...
        # Stock Chart 1
        combo_stock_chart_widget1 = instantiate_combo_chart_widget(df_dict)
        self.tab1 = combo_stock_chart_widget1
        self.tab_widget.addTab(self.tab1, "Stock Chart 1")

        # Stock Chart 2
        combo_stock_chart_widget2 = instantiate_combo_chart_widget(df_dict2)
        self.tab2 = combo_stock_chart_widget2
        self.tab_widget.addTab(self.tab2, "Stock Chart 2")


def main():
    df_dict = {}
    df_dict2 = {}
    symbols_list = ["AAPL", "QQQ", "TSLA"]
    symbols_list2 = ["AMZN", "MSFT", "TSM"]
    period = "180d"
    for symbol in symbols_list:
        df_dict[symbol] = yfinance.download(symbol, period=period)
    for symbol in symbols_list2:
        df_dict2[symbol] = yfinance.download(symbol, period=period)
    app = QApplication.instance()
    if app is None:
        app = QApplication(sys.argv)
    #app = QApplication(sys.argv)
    main_window = MainWindow(None, df_dict, df_dict2)
