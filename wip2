...
def do_load_price_history(symbol, interval,period):    
    ...
    df['Time'] = df['Time'].floordiv(1000000)
...
def add_wicks_markers(ax, highs, lows):
    high_wick = max(highs)
    low_wick = min(lows)
    high_index = highs.index(high_wick)
    low_index = lows.index(low_wick)
    
    # Set the position and label for the high marker
    high_marker_x = high_index
    high_marker_y = high_wick
    high_text_x = high_marker_x
    high_text_y = high_marker_y + 0.1 * (max(highs) - min(lows))
    high_text = f'Highest Wick: {high_wick:.2f}'
    ax.annotate(high_text, xy=(high_marker_x, high_marker_y),
                xytext=(high_text_x, high_text_y), xycoords='data',
                textcoords='data', color='green', alpha=0.7,
                arrowprops=dict(arrowstyle='-', color='green', alpha=0.5))

    # Set the position and label for the low marker
    low_marker_x = low_index
    low_marker_y = low_wick
    low_text_x = low_marker_x
    low_text_y = low_marker_y - 0.1 * (max(highs) - min(lows))
    low_text = f'Lowest Wick: {low_wick:.2f}'
    ax.annotate(low_text, xy=(low_marker_x, low_marker_y),
                xytext=(low_text_x, low_text_y), xycoords='data',
                textcoords='data', color='red', alpha=0.7,
...
def create_ctrl_panel(win):
    ...
    [panel.interval.addItem(i) for i in '1d 4h 1h 30m 15m 5m 1m'.split()] #can't use 1m for 1mo period, cant use 1d 4h for 5d period
    panel.interval.setCurrentIndex(5) # change for number of intervals
    ...
    panel.darkmode.setText('Color mode')
...
add_wicks_markers(ax,highs,lows)
